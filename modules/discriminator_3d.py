...
from modules.util_3d import kp2gaussian
import torch
...
class Discriminator(nn.Module):
    ...
    def __init__(self, scales=(), num_channels=1, block_expansion=32, num_blocks=4, max_features=512,
        ...
        if use_kp:
            for i in range(num_blocks):
                down_blocks.append(
                    DownBlock3d((num_kp+num_channels) if i == 0 else min(max_features, block_expansion * (2 ** i)),
            ...
            for i in range(num_blocks):
                down_blocks.append(
                    DownBlock3d(num_channels if i == 0 else min(max_features, block_expansion * (2 ** i)),
                            min(max_features, block_expansion * (2 ** (i + 1))),
                            norm=(i != 0), kernel_size=4, pool=(i != num_blocks - 1), sn=sn))
